...
class TestCorsMiddlewareProcessRequest(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.middleware = CorsCSRFMiddleware(lambda request: None)
    ...
    def test_disabled_no_cors_headers(self):
        with pytest.raises(MiddlewareNotUsed):
            CorsCSRFMiddleware(lambda request: None)
...
class TestCsrfCrossDomainCookieMiddleware(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.middleware = CsrfCrossDomainCookieMiddleware(lambda request: None)

    @override_settings(FEATURES={'ENABLE_CROSS_DOMAIN_CSRF_COOKIE': False})
    def test_disabled_by_feature_flag(self):
        with pytest.raises(MiddlewareNotUsed):
            CsrfCrossDomainCookieMiddleware(lambda request: None)
